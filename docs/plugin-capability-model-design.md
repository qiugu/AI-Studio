...
>
> **实施进展（M2.0 已落地，2026-09-14）**：本文 §2.5 判定「`plugin_type` 三值无行为差异且边界不可判定」的结论**已被采纳并执行**——`plugin_type` 已从数据模型、接口、前端与文档中整体移除，`source_type` 成为唯一插件形态维度；类型模块更名为 `core/plugin_source_types.py`。故本文中出现的 `core/plugin_types.py` 路径与 `PLUGIN_TYPE_META` 等符号均属**写作时点的历史快照**，现已被删除。落地记录见 [plugin-usage-flow-review.md](plugin-usage-flow-review.md) §8.6。
>
