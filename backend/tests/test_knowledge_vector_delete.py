...
class _StubRepo:
    ...
    def soft_delete_by_doc_id(self, _doc_id, _deleted_at):
        self._events.append("soft_delete_by_doc_id")
        return len(self._chunks)


def _service_with_stubs(chunks):
    events: list = []
    kb = SimpleNamespace(
        id=KB_ID,
        tenant_id=TENANT,
...
class TestDeleteDocumentSelector:
    ...
    def test_chunks_are_soft_deleted_after_vector_cleanup(self, monkeypatch):
        """D9 处置：软删除文档必须级联软删除分块，且顺序在取分块之后
        ...
        monkeypatch.setattr("app.services.knowledge.get_qdrant_client", lambda: MagicMock())

        service.delete_document(DOC_ID)

        assert events == ["list_by_doc_id", "soft_delete_by_doc_id"], (
            "必须先取分块清理向量，再级联软删除分块"
        )
    ...
    def test_failure_is_logged_not_swallowed(self, monkeypatch, caplog):
        ...
        fake_qdrant = MagicMock()
        fake_qdrant.delete.side_effect = RuntimeError("qdrant down")
        monkeypatch.setattr("app.services.knowledge.get_qdrant_client", lambda: fake_qdrant)
